...
class Save extends Action
{
    ...
    public function execute()
    {
        ...
        // Handle time slots from multiselect
            ...
            if (isset($data['block_dates']) && !empty($data['block_dates'])) {
                $blockedDates = $data['block_dates'];
                
                if (is_string($blockedDates)) {
                    if (strpos($blockedDates, '[') === 0) {
                        ...
                        $blockedDatesArray = array_filter($blockedDatesArray); // Remove empty lines
                        $data['block_dates'] = json_encode($blockedDatesArray);
                    }
                } elseif (is_array($blockedDates)) {
                    $data['block_dates'] = json_encode($blockedDates);
                }
            }

            // Add new blocked date if one was selected
            if (isset($data['block_date_picker']) && !empty($data['block_date_picker'])) {
                $newBlockedDate = $data['block_date_picker'];
                
                // Parse existing blocked dates
                $existingBlockedDates = [];
                if (isset($data['block_dates']) && !empty($data['block_dates'])) {
                    if (strpos($data['block_dates'], '[') === 0) {
                        // Parse JSON
                        try {
                            $existingBlockedDates = json_decode($data['block_dates'], true) ?: [];
                        } catch (\Exception $e) {
                            $existingBlockedDates = [];
                        }
                    } else {
                        // Parse newline-separated list
                        $existingBlockedDates = array_map('trim', explode("\n", $data['block_dates']));
                        $existingBlockedDates = array_filter($existingBlockedDates);
                    }
                }
                
                // Add new date if not already in the list
                if (!in_array($newBlockedDate, $existingBlockedDates)) {
                    $existingBlockedDates[] = $newBlockedDate;
                    $data['block_dates'] = json_encode($existingBlockedDates);
                }
            }
