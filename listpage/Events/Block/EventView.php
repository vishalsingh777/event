...
class EventView extends Template
{
     ...
     *
     * @return \Insead\Events\Model\Event
     */
    ...
    public function formatTime($time = null, $format = \IntlDateFormatter::SHORT, $showDate = false)
    {
        if ($time && is_string($time) && strpos($time, ':') !== false) {
            // If it's a time string, use our helper's direct parsing method
    ...
    private function getTimeSlotsFromDatabase($eventId)
    {
        try {
            ...
            $tableName = $this->resourceConnection->getTableName('insead_event_times');
    ...
    public function isMapEnabled()
    {
        return $this->_scopeConfig->isSetFlag(
            'insead_events/google_maps/enabled',
    ...
    public function getGoogleMapsApiKey()
    {
        return $this->_scopeConfig->getValue(
            'insead_events/google_maps/api_key',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    public function getDefaultZoom()
    {
        return (int)$this->_scopeConfig->getValue(
            'insead_events/google_maps/default_zoom',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        ) ?: 15;
    }
}