...
class Success implements HttpGetActionInterface
{
    ...
    public function execute()
    {
        try {
           ...
           // $resultPage->getConfig()->getTitle()->set(__('Registration Confirmed'));
            
            // Set registration and event data for the block
            $block = $resultPage->getLayout()->getBlock('registration.success');
            if ($block) {
            ...
            $this->logger->error('Registration success error: ' . $e->getMessage());
        } catch (\Exception $e) {
            $this->messageManager->addErrorMessage(__('An error occurred while loading the registration information.'));
            $this->logger->error('Registration success exception: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
        }
