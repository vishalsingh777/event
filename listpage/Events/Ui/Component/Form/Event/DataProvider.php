<?php
namespace Insead\Events\Ui\Component\Form\Event;
...
class DataProvider extends AbstractDataProvider
{
    ...
    private function processTimeSlots($event)
    {
        ...
        $timeSlotTable = $this->eventResource->getTable('insead_event_times');
        ...
        if (!empty($timeSlots)) {
            ...
            if ((int)$event->getRecurring() === 0 && !empty($timeSlots)) {
                ...
                if (!empty($dateTimeData)) {
                    $this->loadedData[$eventId]['date_time'] = ['date_time' => $dateTimeData];
    ...
    public function getMeta()
    {
        ...
        $eventUrlPath = 'eventhub/events/'; 
        $dynamicUrlNotice = __('Will be used in the URL to access the event page. Full URL: %1[url_prefix]', $baseUrl . $eventUrlPath);
        
        // Set the notice for the url_key field
        $meta['general_fieldset']['children']['url_key']['arguments']['data']['config']['notice'] = $dynamicUrlNotice;
    } catch (\Exception $e) {
        // If there's an error, we'll fall back to the default notice in the XML
    }
    ...
    private function debugTimeSlots($eventId, $timeSlots)
    {
        ...
        $message .= "Formatted data: " . print_r($this->loadedData[$eventId]['date_time'], true) . "\n";
        $message .= "------------------------------------------------\n";
