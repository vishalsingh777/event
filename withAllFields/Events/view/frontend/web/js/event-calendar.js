    ...
    return function (config) {
        $(document).ready(function () {
                    ...
                    success: function(response) {
                        ...
                        if (response.success && response.data) {
                            ...
                            if (response.data.timeZone) {
                                calendarConfig.timeZone = response.data.timeZone;
                            }
                            ...
                            // For non-recurring events with single time slot
                            if (!response.data.isRecurring && response.data.singleStartTime && response.data.singleEndTime) {
                                calendarConfig.singleStartTime = response.data.singleStartTime;
                                calendarConfig.singleEndTime = response.data.singleEndTime;
                            }
            ...
            function initializeCalendar(config) {
                ...
                $("#event-date-picker").calendar({
                    ...
                    onSelect: function(dateText) {
                        ...
                        // Use the most up to date config when loading time slots
                ...
                // If non-recurring event with single time slot, show that information
                if (!config.isRecurring && config.singleStartTime && config.singleEndTime) {
                    var formattedTime = formatTimeRange(config.singleStartTime, config.singleEndTime);
                    $('#selected-date-display').text($t('This is a single event'));
                    $('.time-selector-prompt').text($t('Event time: ') + formattedTime);
                    
                    // Set the single time slot as selected
                    selectedTimeSlot = config.singleStartTime + '-' + config.singleEndTime;
                    $('#selected-datetime').val(config.eventDate + ' ' + selectedTimeSlot);
                }
            }
            ...
            function loadTimeSlots(dateStr, config) {
                ...
                if (!config.isRecurring && config.singleStartTime && config.singleEndTime) {
                    var formattedTime = formatTimeRange(config.singleStartTime, config.singleEndTime);
                    $container.html('<div class="single-event-time"><p>This event occurs at ' + formattedTime + '</p><div class="time-slot selected" data-time="' + config.singleStartTime + '-' + config.singleEndTime + '">' + formattedTime + '</div></div>');
                    
                    // Store selected date and time
                    selectedTimeSlot = config.singleStartTime + '-' + config.singleEndTime;
                    $('#selected-datetime').val(dateStr + ' ' + selectedTimeSlot);
                    return;
                }
                
                // For recurring events with multiple time slots
                ...
                timeSlots.forEach(function(timeSlot) {
                    var formattedTime = formatTimeSlot(timeSlot);
                ...
                $('.time-slot').on('click', function() {
                    ...
                    $('#selected-datetime').val(selectedDate + ' ' + selectedTimeSlot);
                });
            }
            
            // Format a time slot string (e.g. "09:00-17:00") for display
            ...
            function formatTimeRange(startTime, endTime) {
                ...
                var formattedStart = formatTime(startTime);
                var formattedEnd = formatTime(endTime);
            ...
            // String indexOf function (similar to PHP's strpos)
            ...
            function formatSelectedDateTime(datetime) {
                ...
                try {
                    ...
                    if (timePart.indexOf('-') !== -1) {
                        ...
                        return formattedDate + ' at ' + formatTimeRange(timeParts[0], timeParts[1]);
                    } else {
                        // Single time format
