...
class EventRepository implements EventRepositoryInterface
{
   ...
   /**
    ...
    private function saveTimeSlots($event)
    {
        ...
        $timeSlotTable = $this->resource->getTable('vishal_event_times');
        ...
        // Get the data from the false[date_time] structure
        $falseData = $event->getData('false');
        if (isset($falseData['date_time']) && is_array($falseData['date_time'])) {
            $dateTimeData = $falseData['date_time'];
            ...
            foreach ($dateTimeData as $rowId => $rowData) {
                ...
                // Get the actual field values from the nested 'false' key
                if (isset($rowData['false']) && isset($rowData['false']['time_start']) && isset($rowData['false']['time_end'])) {
                    $timeSlots[] = [
                        'event_id' => $eventId,
                        'time_start' => $rowData['false']['time_start'],
                        'time_end' => $rowData['false']['time_end'],
...
private function loadTimeSlots($event)
{
    ...
    if (!empty($timeSlots)) {
        ...
        // Format data for the form structure with false key and record_id
        $dateTimeData = [];
        $index = 0;
        ...
        $event->setData('false', ['date_time' => $dateTimeData]);
    }
}
